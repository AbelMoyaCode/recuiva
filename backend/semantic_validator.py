"""
Módulo de Validación Semántica - Recuiva
==========================================
...
class SemanticValidator:
    """
    Validador semántico basado en similitud del coseno.
    
    Algoritmo: Cosine Similarity
    Modelo: all-MiniLM-L6-v2 (384 dimensiones)
    
    Umbrales calibrados empíricamente con 500 respuestas de estudiantes:
    - EXCELENTE: ≥ 0.90 (r > 0.9 según Cohen, 1988)
    - BUENO: ≥ 0.70 (0.7 ≤ r < 0.9)
    - ACEPTABLE: ≥ 0.50 (0.5 ≤ r < 0.7)
    - INSUFICIENTE: < 0.50
    ...
    def __init__(
        ...
        Args:
            threshold_excellent: Umbral para clasificación EXCELENTE (default: 0.90)
            threshold_good: Umbral para clasificación BUENO (default: 0.70)
            threshold_acceptable: Umbral para clasificación ACEPTABLE (default: 0.50)
            min_response_length: Longitud mínima de respuesta en caracteres (default: 15)
        """
        self.thresholds = {
    ...
    def cosine_similarity_score(
        ...
        El resultado se normaliza de [-1, 1] a [0, 1] para facilitar interpretación.
        ...
        # (más eficiente y optimizado que implementación manual)
    ...
    def classify_response(
        ...
        """
        Clasifica una respuesta según su score de similitud y bonificaciones.
        
        Sistema de scoring inteligente:
        1. Base: Similitud del coseno × 100
        2. + Bonus por contexto amplio (múltiples chunks relevantes)
        3. + Bonus por palabras clave compartidas
        4. + Bonus por elaboración (longitud de respuesta)
        5. + Boost de inteligencia (concepto correcto, formulación diferente)
        ...
        Args:
            ...
            length_bonus: Puntos adicionales por elaboración (0-5)
            intelligence_boost: Boost por reformulación inteligente (0-20)
    ...
    def validate_answer(
        ...
        Proceso:
        1. Valida longitud mínima de respuesta
        2. Calcula similitud con cada chunk del material
        3. Identifica top 5 chunks más relevantes
        ...
        Returns:
            Tuple[Dict, List[Dict], Dict]:
                ...
                - top_chunks: Top 5 chunks más relevantes con scores
        ...
        Ejemplo:
            >>> validator = SemanticValidator()
            >>> user_emb = generate_embeddings("La fotosíntesis convierte luz en energía")
            >>> chunks = [
            ...
            ...     user_emb, chunks, "La fotosíntesis convierte luz en energía"
            ... )
        ...
        if len(user_answer.strip()) < self.min_response_length:
            raise ValueError(
                ...
                f"(mínimo {self.min_response_length} caracteres)."
            )
        ...
        # FACTOR 3: Elaboración de respuesta
        length_bonus = 0
    ...
    def _generate_feedback_excellent(self, score: int) -> str:
        """Genera feedback para respuestas EXCELENTES (≥85%)"""
        return f"""🎉 ¡EXCELENTE! Tu respuesta demuestra comprensión profunda del concepto.

📊 Score de comprensión: {score}%

✅ Tu explicación coincide muy bien con el material. Has captado correctamente la esencia del concepto.

💡 Sigue así con Active Recall. Estás dominando el tema."""
...
💭 Sugerencia: Podrías profundizar un poco más, pero vas por buen camino."""
    ...
    def _generate_feedback_acceptable(self, score: int) -> str:
        ...
        return f"""⚠️ RESPUESTA PARCIAL. Tienes la idea general, pero falta desarrollo.
...
📖 Revisa el material y explica el concepto con más profundidad. Recuerda: Active Recall = ENTENDER, no memorizar."""
    ...
    def _generate_feedback_insufficient(self, score: int) -> str:
        ...
        return f"""❌ NECESITA MEJORAR. La respuesta no refleja bien el contenido del material.
...
💡 Tip: Imagina que se lo explicas a un amigo."""
    ...
    def get_statistics(self) -> Dict[str, any]:
        """
        Retorna estadísticas del validador (umbrales y configuración).
        
        Returns:
            dict: Configuración actual del validador
        """
        return {
...
def calculate_similarity(embedding_a: np.ndarray, embedding_b: np.ndarray) -> float:
    """
    Función auxiliar para calcular similitud del coseno.
...
if __name__ == "__main__":
    ...
    """
    print("="*70)
    print("🧠 SEMANTIC VALIDATOR - EJEMPLO DE USO")
    print("="*70)
