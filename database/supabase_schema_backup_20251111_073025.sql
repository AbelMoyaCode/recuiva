...
-- Ver chunks del material con problema
SELECT 
    chunk_index,
    chunk_text,
    LENGTH(chunk_text) AS chars
FROM material_embeddings
WHERE material_id = 'c2e9dc80-6abd-4475-9ddf-dc2770441685'
ORDER BY chunk_index
LIMIT 5;
...
-- PASO 1: Ver chunks ANTES de limpiar (primeros 5)
SELECT 
    chunk_index,
    LEFT(chunk_text, 150) AS antes,
    LENGTH(chunk_text) AS longitud_antes
FROM material_embeddings
WHERE material_id = '0394a7f6-cb99-4886-a8e9-0ea05c5d7c56'
ORDER BY chunk_index
LIMIT 5;

-- PASO 2: Aplicar limpieza de OCR
UPDATE material_embeddings
SET chunk_text = (
    -- Paso 2a: Reparar palabras_cortas (1-3 letras)
    -- Ejemplo: "hombr e" → "hombre", "di o" → "dio"
...
UPDATE material_embeddings
SET chunk_text = (
    -- Paso 2b: Reparar palabras_largas (4+ letras minúsculas)
    -- Ejemplo: "di letante" → "diletante", "r encor" → "rencor"
    SELECT regexp_replace(
        chunk_text,
...
UPDATE material_embeddings
SET chunk_text = regexp_replace(chunk_text, '\s+', ' ', 'g')
WHERE material_id = '0394a7f6-cb99-4886-a8e9-0ea05c5d7c56';

-- PASO 4: Ver chunks DESPUÉS de limpiar (primeros 5)
SELECT 
    chunk_index,
    LEFT(chunk_text, 150) AS despues,
    LENGTH(chunk_text) AS longitud_despues
FROM material_embeddings
WHERE material_id = '0394a7f6-cb99-4886-a8e9-0ea05c5d7c56'
ORDER BY chunk_index
LIMIT 5;
...
-- Ver chunks DESPUÉS de la limpieza
SELECT 
    chunk_index,
    LEFT(chunk_text, 150) AS texto_limpio,
